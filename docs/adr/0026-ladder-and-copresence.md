# ADR-0026: 관계는 세 단계, 같은 공간에 있던 것은 같이 논 게 아니다
...
친구는 마주침(같은 곳 30분 + 대화 롤)으로만 생기고(FRIENDS_SPEC §4), 친구가 되면 내 빈 블록을 채워 같이 논다(§2). SNS(ADR-0025)가 생기면
...
5.  **활동 중간에 대화가 성공하면** 앞 컷은 옆모습, 뒤 컷은 정면. 찍을 때 굽기(ADR-0024)라 그대로 남는다.
6.  **"알게 된 것".** 같은 활동마다 하나씩 `memory.friends[id].learned[]`. 친한 친구 프로필에 접기 토글(기본 접힘).

## 근거

*   **사진이 관계를 말한다.** 배경의 옆모습이 며칠 뒤 정면이 되는 걸 사진만 넘겨도 알 수 있다. 이게 벡터를 버린(ADR-0024) 이유이기도 하다.
*   **프라이버시가 규칙에서 나온다.** 배경 인물은 얼굴이 없으니 비공개 계정 사람이 남의 사진에 노출되는 일이 원천적으로 없다. 별도 가림 처리가 필요 없다.
    다만 이 규칙은 **사진에만** 걸린다 (2026-09-11 개정): 방(활동 화면)은 남에게 건너가지 않고 내 캐릭터가 지금 보는 장면이라, 다 같이 등을 돌린 도서관은 그냥 어색하다.
    방에서는 무언가를 마주 보는 자리(카운터·창가·키오스크·거울·설명판·에스컬레이터·부엌·물가)에서만 등을 보이고 나머지는 앞을 본다 (`RoomStage`의 `BACK_SPOTS`). 사진·만화·뷰파인더는 그대로 뒷모습이다.
...
*   `Comic.cast` — 만화가 찍힐 때의 인물 구성(동행·만난 사람+at·같은 공간)을 기억한다. 활동이 KEEP_DAYS 뒤 사라져도 컷을 나중에 구울 때
    (ADR-0024 결정 2) `castOfComic(cast, panel.t)`로 같은 그림. 옛 만화(cast 없음)는 예전 경로(타임라인의 활동에서 되찾기).
