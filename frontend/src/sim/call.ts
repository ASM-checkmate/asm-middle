...
export type CallResult =
  | 'answered'   // 받았다
  | 'missed'     // (수신) 12초가 지났든 안 받기를 눌렀든 지나갔다 — 무슨 얘기였는지는 영영 모른다 (ADR-0004 오너 결정 13)
...
const WORRY_ASK: Record<WorryKey, string> = {
  ...
  focus: '집중이 안 된다며. 뭐가 자꾸 끼어들어?',
  blue: '그냥 안 좋다고 했잖아. 이유 없이 그런 날이지?',
...
const WORRY_PLAN: Record<WorryKey, string[]> = {
  ...
  focus: ['오늘은 한 가지만 하자. 내가 조용한 데로 잡아 놨어.', '머리 식힐 데로 바꿔 놨어. 딴생각 날 틈 없이.'],
  blue: ['그런 날은 아무것도 안 해도 돼. 내가 다 쉬는 걸로 바꿔 놨어.', '오늘은 그냥 옆에 있을게. 멀리 안 가.'],
