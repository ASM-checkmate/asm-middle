...
T = Path('/Users/foxisdog/.claude/jobs/3974294b/tmp'); OUT = T / 'out4'; OUT.mkdir(exist_ok=True); REF = T / 'momo-ref.png'
...
def gen(prompt, paths, out):
    ...
    t = time.perf_counter()
    o = engine.generate_image(seed=7, prompt=prompt, num_inference_steps=STEPS, width=512, height=512, image_paths=[str(x) for x in paths], guidance=4.0,
...
for p in photos:
    ...
    gen(P['C2'], [p], OUT / f'{n}-C2.png'); gen(P['F'], [p], OUT / f'{n}-F.png'); gen(P['G'], [p, REF], OUT / f'{n}-G.png')
cols = ['face', 'C2', 'F', 'G']; S = 300; pad = 8
...
sheet.save(OUT / 'sheet4.png'); print('sheet done', flush=True)
