...
const RoomLab = lazy(async () => {
  ...
  const m = (await load()) as { default?: ComponentType; RoomLab?: ComponentType };
  const C = m.RoomLab ?? m.default;
  if (!C) throw new Error('RoomLab has no component export');
  return { default: C };
});
...
export default function App() {
  ...
  if (LAB === 'room') {
    return (
      <Boundary fallback={<div className="lab-note">방 랩을 열 수 없어요 — src/dev/RoomLab.tsx 를 확인해 주세요.</div>}>
        <Suspense fallback={<div className="lab-note">방 랩 여는 중…</div>}>
          <RoomLab />
        </Suspense>
      </Boundary>
    );
