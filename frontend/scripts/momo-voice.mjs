// 모모 대사를 일레븐랩스 TTS로 만들어 MOMO_TTS_DIR 형식(01.mp3, 02.mp3 …)으로 저장한다 — record.mjs가 그대로 섞는다.
// Usage: ELEVENLABS_API_KEY=… node scripts/momo-voice.mjs <voiceId|목소리 이름> <outDir> [steps.json]
//        ELEVENLABS_API_KEY=… node scripts/momo-voice.mjs --list        # 계정의 목소리 목록
// 순서는 record.mjs와 같다: 대본의 intro → say[1]이 있는 스텝 순. 이모지·기호는 뺀다. 이미 있는 번호는 건너뛴다(다시 만들려면 파일을 지운다).
...
const [arg, outDir = '../design/voice/momo', stepsFile = 'scripts/demo-busan.json'] = process.argv.slice(2);
...
const lines = [];
for (const s of steps) { if (s.intro) lines.push(s.intro); if (s.say && s.say[1]) lines.push(s.say[1]); }
...
console.log(`\n${lines.length}줄 → ${outDir}. 녹화: MOMO_TTS_DIR=${outDir} node scripts/record.mjs /tmp/rec ${stepsFile}`);
