...
**2편은 라이브 시연용이기도 하다 → `scripts/DEMO-LIVE.md`**(URL·로그인·개발 패널 점프·백엔드+Gemini로 진짜 사진 생성). 밤 카드는 에이전트가 '집에서 넷플릭스'를 골라 둔 상태고 시연자(대본)가 코인노래방으로 바꾼다.
...
- **1편 `demo-gwangalli.json`** (에이전트가 잘 노는 것): 첫 화면(모모 손 흔드는 인트로, 말풍선 없이)에서 **나레이션 "나들이는 …"을 화면 중앙 아래 자막으로**(`introShow` 스텝 — 인트로 위에선 `__demoSay`가 옆 패널 대신 인트로 아래 칸에 쓴다, 오너 2026-09-17) → 말풍선 "안녕 나는 모모야" → 부산대(민수) → 지하철 → **삼진포차**(18:37 도착, 저녁·밤 둘 다 포차라 20:00엔 이동 없이 활동만 바뀐다)
  → **"위하여~ 🍻" + 테이블 사진(`pocha-asahi.png`)** → 20:55 드론쇼 → 민수랑 둘이 사진(생성 컷 `public/demo/drone.png` 드롭, 배경 `gwangalli-drone.webp`) → "다음에 또 오자"로 끝. **조새호·해변·시간표·친구 마주침·귀가·SNS 없음**(오너 2026-09-17: 학교에서 바로 포차로, 광안리에서 놀고 끝).
