...
세계·하루·마주침·대화의 시뮬레이션은 전부 여기(`src/sim/`)서 돈다 (ADR-0006 결정 5). 서버 `backend/`(Spring, 8080)는 저장본 동기화·진짜 사람 에이전트·LLM 어댑터만 맡고, 없어도 앱은 오프라인으로 뜬다 (상단 회색 점 "혼자 생각 중"). 카메라의 무대(`src/stage/`, ADR-0014)는 three.js를 카메라를 열 때 동적으로 불러와 그리고, WebGL이 없으면 CSS 무대로 떨어진다.
