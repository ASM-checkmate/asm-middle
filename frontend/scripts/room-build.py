...
# 방 목록은 art/rooms.json 한 군데 (scripts/art.mjs 도 같은 걸 읽는다)
ROOMS = json.load(open('art/rooms.json'))['rooms']

# 장면 = 방 전체 그림 몇 장 (그 물건 + 사람이 함께 그려져 있다). 그림 이름은 방마다의 names 로 채운다.
...
if RID not in ROOMS: sys.exit(f"art/rooms.json 에 '{RID}' 방이 없다 (있는 것: {', '.join(ROOMS)})")
CFG = ROOMS[RID]; N = CFG['names']
...
v = save(N['room'], f'{OUT}/back.jpg')
print(f'  back.jpg  ← {N["room"]}')

scenes = {}
for sid in CFG.get('scenes', list(SCENES)):
    sc = SCENES[sid]
    fmt = lambda f: f.format(**N)
