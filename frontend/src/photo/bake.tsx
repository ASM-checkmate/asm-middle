// ─── 굽기 (ADR-0020 스파이크): 컷 하나를 브라우저에서 픽셀로 ─────────────────────────────────
...
// <use> 세 번(가운데 한 장, 양옆 거울), 캐릭터는 겉모습을 명시해 nested <svg>, 크롭은 <g transform>, 심도는 feGaussianBlur,
...
import {
  DEFAULT_LONG_EDGE, DEFAULT_QUALITY, GHOST_OPACITY, MAX_BYTES, SCENE_CSS, SCENE_VB, STILL_CSS, TOKEN, TOKEN_VARS,
...
/** ShotStage의 props를 하나씩 그대로 — friendColor/metColor/seenColor 의미. ghost는 못 걸어본 사람의 실루엣(GHOST 색, 흐림) */
export interface BakeInput {
  ...
  look: Look;
  /** 동행 (오른쪽 옆에서 손 흔든다) */
  friend?: { color: string; look?: Look };
  /** 말을 건 마주침 상대 (앞쪽) */
  met?: { color: string; look?: Look };
  /** 못 걸어본 사람 — 뒤의 실루엣, 회색·흐림 */
  ghost?: boolean;
}
 ...
 */
function characterSvg(box: Box, pose: Pose, variant: 'me' | 'friend', look: Look | undefined, color?: string): string {
  const raw = renderToStaticMarkup(<Character pose={pose} size={200} variant={variant} look={look} color={color} paused />);
 ...
 * 구조: <defs>(무대 symbol · 필터) + <style>(scenes.css를 hex로 · 캐릭터 정지 자세) + [조도 필터 g] > 종이색 바탕 + [크롭 g] > 무대 ×3 · 실루엣 · 동행 · 나 · 상대
 */
export function bakeSvg(input: BakeInput, opts: BakeSvgOptions = {}): string {
  ...
  const { w, h } = size;
  const cast = { friend: !!input.friend, met: !!input.met, ghost: !!input.ghost };
  ...
  const people: string[] = [];
  if (lay.ghost) people.push(`<g opacity="${GHOST_OPACITY}" filter="url(#ghost)">${characterSvg(lay.ghost, 'idle', 'friend', undefined, GHOST)}</g>`);
  if (lay.friend && input.friend) people.push(fg(characterSvg(lay.friend, 'wave', 'friend', input.friend.look, input.friend.color)));
  people.push(fg(characterSvg(lay.me, input.pose, 'me', input.look)));
  if (lay.met && input.met) people.push(fg(characterSvg(lay.met, 'wave', 'friend', input.met.look, input.met.color)));
