...
export interface ActivityOption {
  ...
  forecast?: string;
  /**
   * 광고 가게 (ADR-0031 확장): 이 카드의 장소가 광고 지면이다 — 시간표 카드·활동 장소 태그에 `AD`. 광고 카드는 일반 카드 사이에 섞이고,
   * 고르는 건 취향이다(이유는 reason에 — 광고라는 말은 안 쓴다). 서버 계약(CONTRACT 옵션)엔 없고 클라이언트·시나리오만 쓴다. 사진·앨범에는 붙지 않는다.
   */
  sponsored?: boolean;
}
