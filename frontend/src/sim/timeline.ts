...
export function buildTimeline(anchor: Anchor, days: Days, memory: Memory, journeys: JourneyCache, horizon: number, encounters: Encounters = {}): ScheduledActivity[] {
  ...
  for (let n = 0; n < MAX_SLOTS; n++) {
    ...
    acts.push({ key, dayKey, blockIds, option: opt, place: place2, fromPlace: cursor.place, journey, departAt, arriveAt, endAt, comicUntil, originTz: tz, tz: destTz, jetlagUntil, companions: opt.friendId ? [opt.friendId] : [], outcome, sketch: plan?.sketch });
